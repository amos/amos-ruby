...
module Amos
  class CreateMerchantApplicationInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Amos::CreateMerchantApplicationInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Amos::CreateMerchantApplicationInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [annual_credit_card_sales_volume, bank_account_number, bank_routing_number, business_category, business_description, city, country_code, customer_service_number, dba_name, legal_entity_application_id, max_transaction_amount, postal_code, primary_contact_email_address, primary_contact_first_name, primary_contact_last_name, primary_contact_phone, state_province, street_address1, street_address2, website_url].hash
    end

...
module Amos
  # Apple Pay card profile for embed confirm. Clients only send wallet_payload (the raw JSON-encoded PKPaymentToken). wallet_provider is derived from the payment method type. Brand, last4, expiration, pan_type, and cryptogram are set from the Vault response. Encrypted PAN and client-supplied cryptogram are not supported. 
  class ApplePayCardProfileInput < ApiModelBase
    # The unmodified JSON-encoded PKPaymentToken.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Amos::ApplePayCardProfileInput` initialize method"
      end

=begin
#Amos Pay API

#Amos Pay API spec

The version of the OpenAPI document: 0.1.0

Generated by: https://openapi-generator.tech
Generator version: 7.23.0

=end

require 'spec_helper'
require 'json'

# Unit tests for Amos::SubscriptionsApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'SubscriptionsApi' do
  before do
    # run before each test
    @api_instance = Amos::SubscriptionsApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of SubscriptionsApi' do
    it 'should create an instance of SubscriptionsApi' do
      expect(@api_instance).to be_instance_of(Amos::SubscriptionsApi)
    end
  end

  # unit tests for cancel_subscription
  # Cancel a subscription by ID
  # @param id The ID of the subscription to cancel
  # @param [Hash] opts the optional parameters
  # @return [Subscription]
  describe 'cancel_subscription test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for create_subscription
  # Create a new subscription
  # @param create_subscription_request 
  # @param [Hash] opts the optional parameters
  # @return [Subscription]
  describe 'create_subscription test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for get_subscription
  # Retrieve a subscription by ID
  # @param id The ID of the subscription to retrieve
  # @param [Hash] opts the optional parameters
  # @return [Subscription]
  describe 'get_subscription test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for list_subscriptions
  # List all subscriptions
  # @param [Hash] opts the optional parameters
  # @option opts [Integer] :page The page of results to retrieve.
  # @option opts [Integer] :per_page Number of results per page.
  # @return [ListSubscriptions]
  describe 'list_subscriptions test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for skip_subscription
  # Skip subscription billing periods by ID
  # @param id The ID of the subscription to skip billing periods for
  # @param skip_subscription_request 
  # @param [Hash] opts the optional parameters
  # @return [Subscription]
  describe 'skip_subscription test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for update_subscription
  # Update a subscription by ID
  # @param id The ID of the subscription to update
  # @param update_subscription_request 
  # @param [Hash] opts the optional parameters
  # @return [Subscription]
  describe 'update_subscription test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
